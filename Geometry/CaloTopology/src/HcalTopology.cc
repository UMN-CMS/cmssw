...
HcalTopology::HcalTopology(HcalTopologyMode::Mode mode, int maxDepthHB,
                           int maxDepthHE, HcalTopologyMode::TriggerMode tmode) :
    excludeHB_(false),
    excludeHE_(false),
    excludeHO_(false),
    excludeHF_(false),
    ...
    firstHORing_(1),
    lastHORing_(15),
    firstHEDoublePhiRing_((mode == HcalTopologyMode::H2
                           || mode == HcalTopologyMode::H2HE) ? (22) : (21)),
    firstHFQuadPhiRing_(40),
    firstHETripleDepthRing_((mode == HcalTopologyMode::H2
                             || mode == HcalTopologyMode::H2HE) ? (24) : (27)),
    singlePhiBins_(72),
    doublePhiBins_(36),
...
bool HcalTopology::valid(const DetId& id) const {
    ...
    if (HcalSubdetector(id.subdetId()) == HcalTriggerTower) {
        return validHT(id);
    }
    return validHcal(id);
}
...
bool HcalTopology::validHT(const HcalTrigTowerDetId& id) const {
    ...
    if (id.iphi() < 1 || id.iphi() > 72 || id.ieta() == 0) {
        return false;
    }
    if (id.version() == 0) {
        if (id.ietaAbs() > 32 || (triggerMode_ == HcalTopologyMode::tm_LHC_1x1
                                  && id.ietaAbs() > 27)) {
            return false;
        }
        if (id.ietaAbs() > 28) {
        ...
        if (id.ietaAbs() < 28 || id.ietaAbs() > 41) {
            return false;
        }
...
bool HcalTopology::isExcluded(const HcalDetId& id) const {
    ...
    if (!exed && !exclusionList_.empty()) {
        std::vector<HcalDetId>::const_iterator i = std::lower_bound(
                                                       exclusionList_.begin(), exclusionList_.end(), id);
...
void HcalTopology::exclude(const HcalDetId& id) {
    std::vector<HcalDetId>::iterator i = std::lower_bound(exclusionList_.begin(),
                                                          exclusionList_.end(), id);
    if (i == exclusionList_.end() || *i != id) {
...
int HcalTopology::exclude(HcalSubdetector subdet, int ieta1, int ieta2,
                          int iphi1, int iphi2, int depth1, int depth2) {
    ...
    for (int ieta = ieta_l; ieta <= ieta_h; ieta++)
        for (int iphi = iphi_l; iphi <= iphi_h; iphi++)
            for (int depth = depth_l; depth <= depth_h; depth++) {
                ...
                if (validRaw(
                            id)) { // use 'validRaw' to include check validity in "uncut" detector
                    exclude(id);
                    n++;
                }
            }
...
bool HcalTopology::validRaw(const HcalDetId& id) const {
    ...
    int depth = id.depth();
    int iphi = id.iphi();
    if ((ieta == 0 || iphi <= 0 || iphi > IPHI_MAX) || aieta > lastHFRing()) {
        return false;    // outer limits
    }
    ...
    if (ok) {
        ...
        if (subdet == HcalBarrel) {
            if (mode_ == HcalTopologyMode::SLHC || mode_ == HcalTopologyMode::H2HE) {
                if ((aieta > lastHBRing() || depth > maxDepthHB_ || (aieta == lastHBRing()
                                                                     && depth > 2))) {
                    ok = false;
                }
            }
            ...
            if (mode_ == HcalTopologyMode::SLHC || mode_ == HcalTopologyMode::H2HE) {
                if (depth > maxDepthHE_ || aieta < firstHERing() || aieta > lastHERing()
                        || (aieta == firstHERing() && depth < 3) || (aieta >= firstHEDoublePhiRing()
                                                                     && (iphi % 2) == 0)) {
                    ok = false;
                }
            }
                ...
                if (depth > 3 || aieta < firstHERing() || aieta > lastHERing()
                        || (aieta == firstHERing() && depth != 3) || (aieta == 17 && depth != 1
                                                                      && mode_ != HcalTopologyMode::H2) || // special case at H2
                        (((aieta >= 17 && aieta < firstHETripleDepthRing()) || aieta == lastHERing())
                         && depth > 2) ||
            ...
            if (aieta < firstHFRing() || aieta > lastHFRing() || ((iphi % 2) == 0)
                    || (depth > 2) || (aieta >= firstHFQuadPhiRing() && ((iphi + 1) % 4) != 0)) {
                ok = false;
            }
        }
        else {
...
int HcalTopology::incAIEta(const HcalDetId& id, HcalDetId neighbors[2]) const {
    ...
    if (aieta == firstHEDoublePhiRing() - 1 && (id.iphi() % 2) == 0) {
        neighbors[0] = HcalDetId(id.subdet(), (aieta + 1) * id.zside(), id.iphi() - 1,
                                 id.depth());
    }
    else if (aieta == firstHFQuadPhiRing() - 1 && ((id.iphi() + 1) % 4) != 0) {
        neighbors[0] = HcalDetId(id.subdet(), (aieta + 1) * id.zside(),
                                 ((id.iphi() == 1) ? (71) : (id.iphi() - 2)), id.depth());
    }
        ...
        neighbors[0] = HcalDetId(id.subdet(), (aieta + 1) * id.zside(), id.iphi(),
                                 id.depth());
    }
...
int HcalTopology::decAIEta(const HcalDetId& id, HcalDetId neighbors[2]) const {
    ...
    if (aieta == firstHEDoublePhiRing()) {
        ...
        neighbors[0] = HcalDetId(id.subdet(), (aieta - 1) * id.zside(), id.iphi(),
                                 id.depth());
        neighbors[1] = HcalDetId(id.subdet(), (aieta - 1) * id.zside(), id.iphi() + 1,
                                 id.depth());
    }
        ...
        if (id.iphi() == IPHI_MAX - 1) {
            ...
            neighbors[1] = HcalDetId(id.subdet(), (aieta - 1) * id.zside(), id.iphi() + 2,
                                     id.depth());
        }
    }
    else if (aieta == 1) {
        neighbors[0] = HcalDetId(id.subdet(), -aieta * id.zside(), id.iphi(),
                                 id.depth());
    }
    else if (aieta == lastHBRing() + 1) {
        neighbors[0] = HcalDetId(HcalBarrel, (aieta - 1) * id.zside(), id.iphi(),
                                 id.depth());
    }
    else if (aieta == lastHERing() + 1) {
        neighbors[0] = HcalDetId(HcalEndcap, (aieta - 1) * id.zside(), id.iphi(),
                                 id.depth());
    }
    else {
        neighbors[0] = HcalDetId(id.subdet(), (aieta - 1) * id.zside(), id.iphi(),
...
void HcalTopology::getDepthSegmentation(unsigned ring,
                                        std::vector<int>& readoutDepths) const {
    ...
    if (pos == depthSegmentation_.begin()) {
        throw cms::Exception("HcalTopology") << "No depth segmentation found for ring"
                                             << ring;
    }
    --pos;
...
std::pair<int, int> HcalTopology::segmentBoundaries(unsigned ring,
    ...
    int d1 = std::lower_bound(readoutDepths.begin(), readoutDepths.end(),
                              depth) - readoutDepths.begin();
    int d2 = std::upper_bound(readoutDepths.begin(), readoutDepths.end(),
                              depth) - readoutDepths.begin();
...
unsigned int HcalTopology::detId2denseIdHB(const DetId& id) const {
    ...
    if (topoVersion_ == 0) {
        ...
        retval = dp - 1 + maxDepthHB_ * (ip - 1) + maxDepthHB_ * 72 *
                 (hid.ieta() - 1 + 33 * zn);
    }
    return retval;
}
...
unsigned int HcalTopology::detId2denseIdHE(const DetId& id) const {
    ...
    if (topoVersion_ == 0) {
        ...
        retval = (dp - 1) + maxDepthHE_ * (ip - 1) + maxDepthHE_ * 72 *
                 (hid.ieta() - 16 + zn * (14 + 29 + 16));
    }
    return retval;
}
...
unsigned int HcalTopology::detId2denseIdCALIB(const DetId& id) const {
    ...
    if (tid.calibFlavor() == HcalCalibDetId::CalibrationBox) {
        ...
        if (abs(ieta) == 4) {
            index = ((iphi - 1) % 36) + (((zside + 1) * 36) / 2) + 72 +
                    425;    //ieta = 1 YB0/HO0;
        }
        else {
            index = (iphi - 1) + (36 * (zside + 1) * 2) +
                    425;    //ieta = 0 for HO2M/HO1M ieta=2 for HO1P/HO2P;
        }
    }
...
unsigned int HcalTopology::detId2denseId(const DetId& id) const {
    ...
    if (topoVersion_ == 0) { // pre-LS1
        ...
        if (hid.subdet() == HcalBarrel) {
            ...
            if (hid.ieta() > 0) {
                retval += (hid.iphi() - 1) + 72 * (hid.ieta() - 1);
            }
            else {
                retval += (hid.iphi() - 1) + 72 * (30 + hid.ieta());
            }
        }
            ...
            if (hid.ieta() > 0) {
                retval += 2 * 72 * (hid.ieta() - 29);
            }
            else {
                retval += 2 * 72 * ((41 + 13) + hid.ieta());
            }
        }
        else {
            return 0xFFFFFFFu;
