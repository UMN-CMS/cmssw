...
void testTriggerGeometry(HcalTopology& topology) {
    ...
    for (int ieta = 1; ieta <= 32; ++ieta) {
        double eta1, eta2;
        const int HF_VERSION = 0; // RCT instead of HF 1x1
        trigTowers.towerEtaBounds(ieta, HF_VERSION, eta1, eta2);
    ...
    if (trigTowers.useRCT()) {
        foward_det_count++;
    }
    if (trigTowers.use1x1()) {
        foward_det_count++;
    }
...
testClosestCells(HcalTopology& topology) {
    edm::FileInPath
    fp("Geometry/HcalTowerAlgo/test/runHcalGeometryAnalyzer_cfg.py");
...
void
testValidDetIds(HcalTopology& topology, DetId::Detector det, int subdet,
                char const* label) {
...
void
testFlexiValidDetIds(HcalTopology& topology, DetId::Detector det, int subdet,
                     char const* label, std::vector<int>& dins) {
    ...
    for (std::vector<int>::const_iterator i = dins.begin(); i != dins.end();
        ...
        std::cout << counter << ": din " << (*i) << " :" << hid << " == " << ihid <<
                  std::endl;
    }
}
...
int main() {
    ...
    testFlexiValidDetIds(stopology, DetId::Hcal, HcalForward, " SLHC FORWARD ",
                         dins);
