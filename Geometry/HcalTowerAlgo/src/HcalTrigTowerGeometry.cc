...
HcalTrigTowerGeometry::towerIds(const HcalDetId& cellId) const {
    ...
    if (cellId.subdet() == HcalForward) {
        ...
        if (useRCT_) {
            ...
            int ieta = firstHFTower(RCT_VERSION);
        ...
        if (use1x1_) {
            ...
            id.setVersion(HF_1X1_VERSION); // version 1 for 1x1 HF granularity
...
HcalTrigTowerGeometry::detIds(const HcalTrigTowerDetId& hcalTrigTowerDetId)
const {
    ...
    if (abs(cell_ieta) <= theTopology->lastHBRing()) {
        theTopology->depthBinInformation(HcalBarrel, abs(tower_ieta), n_depths,
                                         min_depth);
        for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1;
                cell_depth++) {
    ...
    if (abs(cell_ieta) <= theTopology->lastHORing()) {
        theTopology->depthBinInformation(HcalOuter , abs(tower_ieta), n_depths,
                                         min_depth);
        for (int ho_depth = min_depth; ho_depth <= min_depth + n_depths - 1;
                ho_depth++) {
    ...
    if (abs(cell_ieta) >= theTopology->firstHERing() &&
        ...
        theTopology->depthBinInformation(HcalEndcap, abs(tower_ieta), n_depths,
                                         min_depth);
        ...
        if (abs(tower_ieta) == 28) {
            theTopology->depthBinInformation(HcalEndcap, abs(tower_ieta) + 1, n_depths,
                                             min_depth);
            for (int cell_depth = min_depth; cell_depth <= min_depth + n_depths - 1;
                    cell_depth++) {
                if (tower_ieta < 0) {
                    results.push_back(HcalDetId(HcalEndcap, tower_ieta - 1, cell_iphi,
                                                cell_depth));
                }
                if (tower_ieta > 0) {
                    results.push_back(HcalDetId(HcalEndcap, tower_ieta + 1, cell_iphi,
    ...
    if (abs(cell_ieta) >= theTopology->firstHFRing()) {
        ...
        if (hcalTrigTowerDetId.version() == RCT_VERSION) {

            int HfTowerPhiSize =  72 / nPhiBins(tower_ieta, RCT_VERSION);

            int HfTowerEtaSize     = hfTowerEtaSize(tower_ieta);
            int FirstHFRingInTower = firstHFRingInTower(abs(tower_ieta));

            for (int iHFTowerPhiSegment = 0; iHFTowerPhiSegment < HfTowerPhiSize;
                    iHFTowerPhiSegment++) {

                cell_iphi = (tower_iphi / HfTowerPhiSize) *
                            HfTowerPhiSize;  // Find the minimum phi segment
                cell_iphi -=
                    2; // The first trigger tower starts at HCAL iphi = 71, not HCAL iphi = 1
                cell_iphi +=
                    iHFTowerPhiSegment;       // Get all of the HCAL iphi values in this trigger tower
                cell_iphi +=
                    72;                       // Don't want to take the mod of a negative number
                ...
                for (int iHFTowerEtaSegment = 0; iHFTowerEtaSegment < HfTowerEtaSize;
                        iHFTowerEtaSegment++) {
        ...
        else if (hcalTrigTowerDetId.version() == HF_1X1_VERSION) {
...
int HcalTrigTowerGeometry::hfTowerEtaSize(int ieta) const {
    ...
    assert(ietaAbs >= firstHFTower(RCT_VERSION) && ietaAbs <= nTowers(RCT_VERSION));
    // the first three come from rings 29-31, 32-34, 35-37. The last has 4 rings: 38-41
    return (ietaAbs == nTowers(RCT_VERSION)) ? 4 : 3;
...
int HcalTrigTowerGeometry::firstHFRingInTower(int ietaTower) const {
    ...
    for (int iTower = firstHFTower(RCT_VERSION); iTower != inputTower; ++iTower) {
...
void HcalTrigTowerGeometry::towerEtaBounds(int ieta, int version, double& eta1,
                                           double& eta2) const {
    int ietaAbs = abs(ieta);
    if (ietaAbs < firstHFTower(version)) {
        eta1 = theHBHEEtaBounds[ietaAbs - 1];
        eta2 = theHBHEEtaBounds[ietaAbs];
        // the last tower is split, so get tower 29, too (only in the RCT version)
        if (ietaAbs == theTopology->lastHERing() - 1 && version == RCT_VERSION) {
            eta2 = theHBHEEtaBounds[ietaAbs + 1];
        }
    }
    else {
        if (version == RCT_VERSION) {
            ...
            if (ietaAbs == 29) {
                ietaAbs = 30;    // HF 29 is a tail catcher/guard-ring.  should not define eta for TT
            }
