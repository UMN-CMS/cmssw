...
void
HcalTopologyIdealEP::fillDescriptions(edm::ConfigurationDescriptions&
                                      descriptions) {
    ...
    hcalTopologyConstants.add<std::string>("triggerMode",
    ...
    hcalSLHCTopologyConstants.add<std::string>("triggerMode",
                                               "HcalTopologyMode::tm_LHC_1x1");

    edm::ParameterSetDescription desc;
    desc.addUntracked<std::string>("Exclude", "");
    desc.addOptional<edm::ParameterSetDescription>("hcalTopologyConstants",
    ...
    descSLHC.addOptional<edm::ParameterSetDescription>("hcalTopologyConstants",
...
HcalTopologyIdealEP::produce(const IdealGeometryRecord& iRecord) {
    ...
    if (m_pSet.exists("hcalTopologyConstants")) {
        const edm::ParameterSet hcalTopoConsts(
            m_pSet.getParameter<edm::ParameterSet>("hcalTopologyConstants"));
        StringToEnumParser<HcalTopologyMode::Mode> eparser;
        mode = (HcalTopologyMode::Mode) eparser.parseString(
                   hcalTopoConsts.getParameter<std::string>("mode"));
        ...
        tmode = (HcalTopologyMode::TriggerMode) tparser.parseString(
                    hcalTopoConsts.getParameter<std::string>("triggerMode"));

    }
    //  std::cout << "mode = " << mode << ", maxDepthHB = " << maxDepthHB << ", maxDepthHE = " << maxDepthHE << std::endl;
    edm::LogInfo("HCAL") << "mode = " << mode << ", maxDepthHB = " << maxDepthHB <<
                         ", maxDepthHE = " << maxDepthHE << "trigger mode=" << tmode;
    ...
    for (std::vector<RingSegmentation>::const_iterator ringSegItr =
