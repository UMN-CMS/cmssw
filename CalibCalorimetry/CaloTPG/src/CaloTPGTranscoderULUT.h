...
class CaloTPGTranscoderULUT : public CaloTPGTranscoder {
    public:
        ...
        virtual HcalTriggerPrimitiveSample hcalCompress(const HcalTrigTowerDetId& id,
                                                        unsigned int sample, bool fineGrain, HcalTrigTowerGeometry const&) const;
        virtual EcalTriggerPrimitiveSample ecalCompress(const EcalTrigTowerDetId& id,
                                                        unsigned int sample, bool fineGrain) const;
        ...
        virtual double hcaletValue(const HcalTrigTowerDetId& hid,
                                   const HcalTriggerPrimitiveSample& hc, HcalTrigTowerGeometry const&) const;
        virtual std::vector<unsigned char> getCompressionLUT(HcalTrigTowerDetId id,
                                                             HcalTopology const&) const;
    ...
    private:
        ...
        // Member functions
        void loadHCALCompress(HcalLutMetadata const&,
                              HcalTrigTowerGeometry const&) ; //Analytical compression tables
        void loadHCALCompress(const std::string& filename, HcalLutMetadata const&,
                              HcalTrigTowerGeometry const&) ; //Compression tables from file
        void loadHCALUncompress(HcalLutMetadata const&,
                                HcalTrigTowerGeometry const&) ; //Analytical decompression
        void loadHCALUncompress(const std::string& filename, HcalLutMetadata const&,
                                HcalTrigTowerGeometry const&) ; //Decompression tables from file
