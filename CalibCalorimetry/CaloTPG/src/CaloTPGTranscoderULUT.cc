...
void CaloTPGTranscoderULUT::loadHCALCompress(HcalLutMetadata const& lutMetadata,
    ...
    if (OUTPUT_LUT_SIZE != (unsigned int) 0x400) {
        std::cout << "Error: Analytic compression expects 10-bit LUT; found LUT with "
                  << OUTPUT_LUT_SIZE << " entries instead" << std::endl;
    }
    ...
    for (int ieta = -32; ieta <= 32; ieta++) {
        ...
        for (int iphi = 1; iphi <= 72; iphi++) {
            ...
            if (outputLUT_[lutId] != 0) {
                std::cout << "Error: LUT with (ieta,iphi) = (" << ieta << "," << iphi <<
            ...
            for (unsigned int i = threshold; i < OUTPUT_LUT_SIZE; ++i) {
                outputLUT_[lutId][i] = (abs(ieta) < theTrigTowerGeometry.firstHFTower(
                                            id.version())) ? analyticalLUT[i] : identityLUT[i];
            }
        } //for iphi
    } //for ieta
    // Version 1 loop
...
void CaloTPGTranscoderULUT::loadHCALCompress(const std::string& filename,
    ...
    std::cout << "Initializing compression LUT's from " << (char*)filename.data() <<
              std::endl;
    for (int i = 0; i < NOUTLUTS; i++) {
        outputLUT_[i] = 0;
    }
    ...
    for (int ieta = -32; ieta <= 32; ieta++) {
        for (int iphi = 1; iphi <= 72; iphi++) {
            ...
            if (topo.validHT(id)) {
                ...
                if (outputLUT_[rawid] != 0) {
                    std::cout << "Error: LUT with (ieta,iphi) = (" << ieta << "," << iphi <<
                              ") has been previously allocated!" << std::endl;
                }
                else {
                    outputLUT_[rawid] = new LUT[OUTPUT_LUT_SIZE];
                }
                if (rawid < minid) {
                    minid = rawid;
                }
    ...
    if (userfile) {
        ...
        for (unsigned int i = 0; i <= s.length(); i++) {
            userfile.unget();    //rewind last line
        }
        outputluts.resize(nluts);
        for (int i = 0; i < nluts; i++) {
            outputluts[i].resize(OUTPUT_LUT_SIZE);
        }
        ...
        for (unsigned int j = 0; j < OUTPUT_LUT_SIZE; j++) {
            for (int i = 0; i < nluts; i++) {
                userfile >> tool;
                if (tool < 0) {
                    std::cout << "Error: LUT can't have negative numbers; 0 used instead: " << i <<
                              ", " << j << " : = " << tool << std::endl;
                    tool = 0;
                }
                else if (tool > 0xff) {
                    std::cout << "Error: LUT can't have >8-bit numbers; 0xff used instead: " << i <<
                              ", " << j << " : = " << tool << std::endl;
                    tool = 0xff;
                }
                outputluts[i][j] = tool;
                if (userfile.eof()) {
                    std::cout << "Error: LUT file is truncated or has a wrong format: " << i << ","
                              << j << std::endl;
                }
            }
        }
            ...
            for (int iphi = loiphi[i]; iphi <= hiiphi[i]; iphi++) {
                for (int ieta = loieta[i]; ieta <= hiieta[i]; ieta++) {
                    ...
                    if (topo.validHT(detid)) {
                        ...
                        if (outputLUT_[id] == 0) {
                            throw cms::Exception("PROBLEM: outputLUT_ has not been initialized for ieta, iphi, id = ")
                                    << ieta << ", " << iphi << ", " << id << std::endl;
                        }
                        for (int j = 0; j <= 0x3ff; j++) {
                            outputLUT_[id][j] = outputluts[i][j];
                        }
                        nini++;
                        ntot++;
                    }
...
void CaloTPGTranscoderULUT::loadHCALUncompress(HcalLutMetadata const&
                                               lutMetadata,
                                               HcalTrigTowerGeometry const& theTrigTowerGeometry) {
    hcaluncomp_.clear();
    for (int i = 0; i < NOUTLUTS; i++) {
    ...
    for (int ieta = -32; ieta <= 32; ++ieta) {
        ...
        theTrigTowerGeometry.towerEtaBounds(ieta, version_of_hcal_TPs, eta_low,
                                            eta_high);
...
HcalTriggerPrimitiveSample CaloTPGTranscoderULUT::hcalCompress(
    const HcalTrigTowerDetId& id, unsigned int sample, bool fineGrain,
    HcalTrigTowerGeometry const& theTrigTowerGeometry) const {

    int itower = theTrigTowerGeometry.topology().detId2denseIdHT(id);

    if (itower < 0) {
        cms::Exception("Invalid Data") << "No trigger tower found for ieta, iphi = " <<
                                       id.ieta() << ", " << id.iphi() << " v" << id.version();
    }
    if (sample >= OUTPUT_LUT_SIZE) {

        throw cms::Exception("Out of Range") << "LUT has 1024 entries for " << itower <<
                                             " but " << sample << " was requested.";
...
EcalTriggerPrimitiveSample CaloTPGTranscoderULUT::ecalCompress(
    const EcalTrigTowerDetId& id, unsigned int sample, bool fineGrain) const {
...
std::vector<unsigned char> CaloTPGTranscoderULUT::getCompressionLUT(
    HcalTrigTowerDetId id, HcalTopology const& topology) const {
    std::vector<unsigned char> lut;
    int itower = topology.detId2denseIdHT(id);
    if (itower >= 0) {
        lut.resize(OUTPUT_LUT_SIZE);
        for (unsigned int i = 0; i < OUTPUT_LUT_SIZE; i++) {
            lut[i] = outputLUT_[itower][i];
        }
    }
    return lut;
}
...
void CaloTPGTranscoderULUT::setup(HcalLutMetadata const& lutMetadata,
    ...
    if (rctlsb != 0.25 && rctlsb != 0.5) {
        throw cms::Exception("RCTLSB") << " value=" << rctlsb <<
