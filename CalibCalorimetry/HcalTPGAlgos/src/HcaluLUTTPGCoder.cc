...
void HcaluLUTTPGCoder::compress(const IntegerCaloSamples& ics,
                                const std::vector<bool>& featureBits, HcalTriggerPrimitiveDigi& tp) const {
...
int HcaluLUTTPGCoder::getLUTId(HcalSubdetector id, int ieta, int iphi,
                               int depth) const {
    int detid = 0;
    if (id == HcalEndcap) {
        detid = 1;
...
void HcaluLUTTPGCoder::update(const char* filename, const HcalTopology& theTopo,
                              bool appendMSB) {
        ...
        for (int iphi = 1; iphi <= 72; ++iphi) {
            for (int depth = 1; depth <= 3; ++depth) {
                for (int isub = 0; isub < 3; ++isub) {
                    ...
                    if (!theTopo.valid(detid)) {
                        continue;
                    }
                    int id = getLUTId(subdet[isub], ieta, iphi, depth);
                    std::vector<unsigned int>* lut = _xml->getLutFast(detid);
                    if (lut == 0) {
                        throw cms::Exception("PROBLEM: No inputLUT_ in xml file for ") << detid <<
                                                                                       std::endl;
                    }
                    if (lut->size() != 128) {
                        throw cms::Exception("PROBLEM: Wrong inputLUT_ size in xml file for ") << detid
                                                                                               << std::endl;
                    }
                    for (int i = 0; i < 128; ++i) {
                    ...
                    if (subdet == HcalBarrel || subdet == HcalEndcap) {
                        ...
                        for (int adc = 0; adc <= 0x7F; ++adc) {
                            ...
                            if (isMasked) {
                                ...
                                inputLUT_[lutId][adc] = (LutElement) std::min(std::max(0,
                                                                                       int((adc2fC - ped) * gain * rcalib / nominalgain_ / granularity)), 0x3FF);
                            }
                        }
                    }  // endif HBHE
                        ...
                        for (int capId = 0; capId < 4; ++capId) {
                            one_adc2fC += channelCoder->charge(*shape, 1,
                                                               capId) - channelCoder->charge(*shape, 0, capId);
                        }
                        one_adc2fC /= 4.0;
                        ...
                        for (int adc = 0; adc <= 0x7F; ++adc) {
                            ...
                            if (isMasked) {
                                ...
                                inputLUT_[lutId][adc] = std::min(std::max(0,
                                                                          int((adc2fC - ped + offset) * gain * rcalib / lsb_ / cosh_ieta[abs(ieta)])),
                                                                 0x3FF);
                            }
                        }
                    } // endif HF
