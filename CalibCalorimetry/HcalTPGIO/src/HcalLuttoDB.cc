...
class HcalLuttoDB : public edm::EDAnalyzer {
    ...
    private:
        void writeoutlut1(HcalDetId id, HcalElectronicsId eid,
                          const std::vector<unsigned short>& lut, std::ostream& os);
        std::vector<unsigned char> extractOutputLut(const CaloTPGTranscoder& coder,
                                                    HcalTrigTowerDetId chan, HcalTrigTowerGeometry const& theTrigTowerGeometry);
        void writeoutlut2(HcalTrigTowerDetId id, HcalElectronicsId eid,
                          const std::vector<unsigned char>& lut, std::ostream& os);
        bool filePerCrate_;
...
std::ostream* HcalLuttoDB::openPerLut1(HcalElectronicsId eid) {
    char fname[1024];
    snprintf(fname, 1024, "%s_%d_%d%c_%d_%d_1.xml", fileformat_.c_str(),
             eid.readoutVMECrateId(),
             eid.htrSlot(), ((eid.htrTopBottom()) ? ('t') : ('b')), eid.fiberIndex(),
             eid.fiberChanId());
    std::ostream* os = new std::ofstream(fname);
    (*os) << "<?xml version=\"1.0\"?>\n";
    return os;
...
std::ostream* HcalLuttoDB::openPerLut2(HcalElectronicsId eid) {
    char fname[1024];
    snprintf(fname, 1024, "%s_%d_%d%c_%d_%d_2.xml", fileformat_.c_str(),
             eid.readoutVMECrateId(),
             eid.htrSlot(), ((eid.htrTopBottom()) ? ('t') : ('b')), eid.slbSiteNumber(),
             eid.slbChannelIndex());
    std::ostream* os = new std::ofstream(fname);
    (*os) << "<?xml version=\"1.0\"?>\n";
    return os;
...
void
HcalLuttoDB::writeoutlut1(HcalDetId id, HcalElectronicsId eid,
                          const std::vector<unsigned short>& lut, std::ostream& os) {

    os << "<CFGBrick> " << std::endl;
    os << " <Parameter name='IETA' type='int'>" << id.ieta() << "</Parameter>" <<
       std::endl;
    os << " <Parameter name='IPHI' type='int'>" << id.iphi() << "</Parameter>" <<
       std::endl;
    os << " <Parameter name='DEPTH' type='int'>" << id.depth() << "</Parameter>" <<
       std::endl;
    os << " <Parameter name='CRATE' type='int'>" << eid.readoutVMECrateId() <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='SLOT' type='int'>" << eid.htrSlot() << "</Parameter>"
       << std::endl;
    os << " <Parameter name='TOPBOTTOM' type='int'>" << eid.htrTopBottom() <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='FIBER' type='int'>" << eid.fiberIndex() <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='FIBERCHAN' type='int'>" << eid.fiberChanId() <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='LUT_TYPE' type='int'>1</Parameter>" << std::endl;
    os << " <Parameter name='CREATIONTAG' type='string'>" << creationtag_ <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='CREATIONSTAMP' type='string'>" << creationstamp_ <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='FORMATREVISION' type='string'>" << formatRevision_ <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='TARGETFIRMWARE' type='string'>" << targetfirmware_ <<
       "</Parameter>" << std::endl;
    int generalizedIndex = id.ietaAbs() + 1000 * id.depth() + 10000 * id.iphi() +
                           ((id.ieta() < 0) ? (0) : (100)) + ((id.subdet() == HcalForward
                                                               && id.ietaAbs() == 29) ? (4 * 10000) : (0));

    os << " <Parameter name='GENERALIZEDINDEX' type='int'>" << generalizedIndex <<
       "</Parameter>" << std::endl;
    // do checksum
    md5_state_t md5er;
...
void
HcalLuttoDB::writeoutlut2(HcalTrigTowerDetId id, HcalElectronicsId eid,
                          const std::vector<unsigned char>& lut, std::ostream& os) {
    ...
    os << " <Parameter name='TOPBOTTOM' type='int'>" << eid.htrTopBottom() <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='SLB' type='int'>" << eid.slbSiteNumber() <<
       "</Parameter>" << std::endl;
    os << " <Parameter name='SLBCHAN' type='int'>" << eid.slbChannelIndex() <<
       "</Parameter>" << std::endl;
...
std::vector<unsigned char> HcalLuttoDB::extractOutputLut(
    const CaloTPGTranscoder& coder, HcalTrigTowerDetId chan,
    ...
    for (int crate = 0; crate < 20; crate++) {
        ...
        for (itreid  = allEID.begin(); itreid != allEID.end(); itreid++) {
            ...
            if (itreid->isTriggerChainId()) { // lut2
                ...
                std::vector<unsigned short> lut = inputCoder->getLinearizationLUT(HcalDetId(
                                                                                      gid));
