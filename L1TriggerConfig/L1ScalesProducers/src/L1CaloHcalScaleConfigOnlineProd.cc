...
L1CaloHcalScaleConfigOnlineProd::L1CaloHcalScaleConfigOnlineProd(
    ...
    if (iConfig.exists("hcalTopologyConstants")) {
        const edm::ParameterSet hcalTopoConsts =
            iConfig.getParameter<edm::ParameterSet>("hcalTopologyConstants");
        StringToEnumParser<HcalTopologyMode::Mode> parser;
        mode = (HcalTopologyMode::Mode) parser.parseString(
...
L1CaloHcalScaleConfigOnlineProd::~L1CaloHcalScaleConfigOnlineProd() {
    ...
    if (theTrigTowerGeometry != 0) {
        delete theTrigTowerGeometry;
    }
}
    ...
    for (int i = 0; i < chanResults.numberRows() ; ++i) {
        ...
        if (objectName == "HcalTrigTowerDetId") { //trig tower
            ...
            for (unsigned int k = threshold; k < 1024; ++k) {
                outputLut[k] = (abs(ieta) < theTrigTowerGeometry->firstHFTower(
                                    version_of_hcal_TPs)) ? analyticalLUT[k] : identityLUT[k];
            }
