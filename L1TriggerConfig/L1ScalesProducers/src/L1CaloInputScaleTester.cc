...
void
L1CaloInputScaleTester::analyze(const edm::Event& iEvent,
    ...
    for (unsigned short input = 0; input <= 0xFF; input++) {
        ...
        for (unsigned short absIeta = 1; absIeta <= 32; absIeta++) {
            hcal1 = caloTPGTranscoder->hcaletValue(absIeta, input,
                                                   *theTrigTowerGeometry); // no eta-
            hcal2 = caloHcalScale->et(input, absIeta, 1); // sign in transcoder
            hcal3 = caloTPGTranscoder->hcaletValue(-absIeta, input,
                                                   *theTrigTowerGeometry); // no eta-
