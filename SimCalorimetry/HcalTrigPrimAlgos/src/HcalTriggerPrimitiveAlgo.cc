...
void HcalTriggerPrimitiveAlgo::run(const HcalTPGCoder* incoder,
    ...
    for (SumMap::iterator mapItr = theSumMap.begin(); mapItr != theSumMap.end();
            ++mapItr) {
...
void HcalTriggerPrimitiveAlgo::addSignal(const HBHEDataFrame& frame) {
    ...
    std::vector<HcalTrigTowerDetId> ids = theTrigTowerGeometry->towerIds(
                                              frame.id());
        ...
        for (it = ids.begin(); it != ids.end(); ++it) {
            ...
            if (trig_tower_id.version() == 0) {
                ...
                if (theTowerMapFGSum.find(trig_tower_id) == theTowerMapFGSum.end()) {
                    ...
                    theTowerMapFGSum.insert(std::pair<HcalTrigTowerDetId, SumFGContainer>
                                            (trig_tower_id, sumFG));
                }
    ...
    for (int ibin = 0; ibin < numberOfSamples_; ++ibin) {
        ...
        if (peakfind_) {
            ...
            switch (peak_finder_algorithm_) {
                case 1 :
                    isPeak = (samples[idx] > samples[idx - 1] && samples[idx] >= samples[idx + 1]
                              && samples[idx] > theThreshold);
                    break;
                case 2:
                    isPeak = (sum[idx] > sum[idx - 1] && sum[idx] >= sum[idx + 1]
                              && sum[idx] > theThreshold);
                    break;
                default:
                    break;
            }
...
void HcalTriggerPrimitiveAlgo::analyzeHF(IntegerCaloSamples& samples,
                                         HcalTriggerPrimitiveDigi& result, const int hf_lumi_shift) {
    std::vector<bool> finegrain(numberOfSamples_, false);
    ...
    for (SumFGContainer::const_iterator sumFGItr = sumFG.begin();
            sumFGItr != sumFG.end(); ++sumFGItr) {
...
void HcalTriggerPrimitiveAlgo::analyzeHFV1(
    ...
    for (int ibin = 0; ibin < numberOfSamples_; ++ibin) {
        ...
        if (HCALFEM != 0) {
            finegrain[ibin] = HCALFEM->fineGrainbit(ADCShort, HF_DETAILS->ShortDigi.id(),
                                                    HF_DETAILS->ShortDigi[ibin].capid(), ADCLong, HF_DETAILS->LongDigi.id(),
                                                    HF_DETAILS->LongDigi[ibin].capid());
        }
    }
    outcoder_->compress(output, finegrain, result, *theTrigTowerGeometry);
...
void HcalTriggerPrimitiveAlgo::runZS(HcalTrigPrimDigiCollection& result) {
    for (HcalTrigPrimDigiCollection::iterator tp = result.begin();
            tp != result.end(); ++tp) {
        bool ZS = true;
...
void HcalTriggerPrimitiveAlgo::runFEFormatError(const FEDRawDataCollection*
                                                rawraw,
    ...
    for (int i = FEDNumbering::MINHCALFEDID; i <= FEDNumbering::MAXHCALFEDID; ++i) {
        ...
        for (int spigot = 0; spigot < HcalDCCHeader::SPIGOT_COUNT; spigot++) {
            ...
            bool HTRError = (!htr.check() || htr.isHistogramEvent()
                             || (errWord & 0x800) != 0);
            ...
            if (HTRError) {
                ...
                for (int fchan = 0; fchan < 3 && !valid; fchan++) {
                    for (int fib = 0; fib < 9 && !valid; fib++) {
                        ...
                        for (std::vector<HcalTrigTowerDetId>::const_iterator triggerId = ids.begin();
                                triggerId != ids.end(); ++triggerId) {
