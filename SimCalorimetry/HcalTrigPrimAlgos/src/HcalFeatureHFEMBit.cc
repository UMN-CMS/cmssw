...
HcalFeatureHFEMBit::HcalFeatureHFEMBit(double ShortMinE, double LongMinE,
                                       double ShortLongCutSlope, double ShortLongCutOffset,
...
bool HcalFeatureHFEMBit::fineGrainbit(int ADCShort, HcalDetId Sid, int CapIdS,
                                      int ADCLong, HcalDetId Lid, int CapIdL) const { //pass det id
    ...
    if ((ShortE < ((LongE) - (ShortLongCutOffset_)) * ShortLongCutSlope_)
            && LongE > LongMinE_ && ShortE > ShortMinE_) {
        return true;
    }
    else {
        return false;
