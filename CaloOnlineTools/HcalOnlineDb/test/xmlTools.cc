...
int createLMap(void);
int createRBXLoader(std::string& type_, std::string& tag_,
                    std::string& list_file, std::string& _comment, std::string& _version);
...
int createZSLoader2(std::string& tag, std::string& comment, std::string& zs2HB,
                    std::string& zs2HE, std::string& zs2HO, std::string& zs2HF);
...
int main(int argc, char** argv) {
    ...
    ("test-lut-xml-file-access",
     "test and benchmark LUT reading from local XML file")
    ...
    ("version-name", po::value<string>(&version_name)->default_value("V00-01-01"),
     "version name")
    ("sub-version", po::value<int>(&sub_version)->default_value(1),
     "sub-version number")
    ("file-list", po::value<string>(), "list of files for further processing")
    ("crate", po::value<int>(&crate)->default_value(-1), "crate number")
    ("lut-type", po::value<int>(&crate)->default_value(1),
     "LUT type: 1 - linearization, 2 - compression")
    ...
    ("create-lut-xml-lin-ascii-comp-coder",
     "create XML file(s) with linearizer LUTs from ASCII master and compression LUTs from coder")
    ("create-lut-loader",
     "create XML database loader for LUTs, and zip everything ready for uploading to the DB")
    ("create-trigger-key", "create a trigger key entry")
    ("get-lut-xml-from-oracle", "Get LUTs from Oracle database")
    ("database-accessor", po::value<string>
     (&db_accessor)->default_value("occi://CMS_HCL_PRTTYPE_HCAL_READER@anyhost/int2r?PASSWORD=HCAL_Reader_88,LHWM_VERSION=22"),
     "Database accessor std::string")
    ("lin-lut-master-file", po::value<string>(),
     "Linearizer LUT ASCII master file name")
    ("comp-lut-master-file", po::value<string>(),
     "Compression LUT ASCII master file name")
    ("do-not-split-by-crate",
     "output LUTs as a single XML instead of making a separate file for each crate")
    ...
    ("get-iovs-from-omds",
     "For a given tag, get the list of unique IOVs ordered by insertion time")
    ("test-tags-iovs-from-pool",
     "Test the extraction of lists of tags and IOVs from a Pool database")
    ("list-iovs-for-o2o",
     "For a given tag, dump a list of IOVs that need to be copied via O2O or report error if O2O is impossible")
    ("pool-connect-string", po::value<string>(),
     "Connect string to the Pool database")
    ("pool-auth-path", po::value<string>(),
     "Path to authenticate.xml for the Pool database")
    ("make-channel-quality-xml",
     "Create channel quiality loader XML with all channels")
    ;
    ...
    try {
        ...
        if (vm.count("quicktest")) {
            // db;
            std::cout << "HcalDbOmds: cms_hcal_dcs_02:HCAL_HV/HVcrate_HEMC/S17/RM4HV: " <<
                      HcalDbOmds::getDcsTypeFromDpName("cms_hcal_dcs_02:HCAL_HV/HVcrate_HEMC/S17/RM4HV")
                      << std::endl;
            std::cout << "HcalDbOmds: cms_hcal_dcs_02:HCAL_HV/HVcrate_HO2P/S03/RM1BV: " <<
                      HcalDbOmds::getDcsTypeFromDpName("cms_hcal_dcs_02:HCAL_HV/HVcrate_HO2P/S03/RM1BV")
                      << std::endl;
            return 0;
        }
        ...
        if (vm.count("test-lut-xml-file-access")) {
            ...
            gettimeofday(&_t, NULL);
            std::cout << "Initialization took: " << (double)(_t . tv_sec) + (double)(
                          _t . tv_usec) / 1000000.0 - _time << "sec" << std::endl;
            _xml->create_lut_map();
        ...
        if (vm.count("test-channel-quality")) {
            ...
            std::cout << "Channels quality obtained from OMDS for " << nChan << " channels"
                      << std::endl;
            for (std::map<int, HcalChannelQualityXml::ChannelQuality>::const_iterator cq =
                        xml.geomid_cq.begin();
        ...
        if (vm.count("test-channel-iterator")) {
            ...
            std::cout << "The iterator list contains " << iter.size() <<
                      " entries (not necessarily unique)" << std::endl;
        ...
        if (vm.count("list-iovs-for-o2o")) {
            if (!vm.count("tag-name")) {
                ...
                if (n_iovs == -1) {
                    ...
                    std::cout << "HcalO2OManager: O2O is not needed, the tag is up to date" <<
                              std::endl;
                }
                else {
            ...
            std::cout << "This should never be printed out. Something is fishy!" <<
                      std::endl;
        }
        ...
        std::cout <<
                  "No command line options known to boost... continuing to getopt parser..." <<
                  std::endl;
    }
    ...
    */
    ...
    //}
        ...
        while (1) {
            ...
            if (!zs2HB_b) {
                std::cout << "No zero suppression value dor HB specified... exiting" <<
                          std::endl;
                break;
            }
            if (!zs2HE_b) {
                std::cout << "No zero suppression value dor HE specified... exiting" <<
                          std::endl;
                break;
            }
            if (!zs2HO_b) {
                std::cout << "No zero suppression value dor HO specified... exiting" <<
                          std::endl;
                break;
            }
            if (!zs2HF_b) {
                std::cout << "No zero suppression value dor HF specified... exiting" <<
                          std::endl;
                break;
            }
...
int createLUTLoader(std::string _prefix, std::string tag_name) {
    ...
    baseConf . comment_description =
        "Version 2 (HO bug fixed, now input and output LUTs for HO should be in.) Input: pedestal compensation by 3 for HB, HE, HF, and HO, nominal linearization, output: |ieta|<=14 thr=7 (equal or more), |ieta|=15 thr=9, |ieta|=16 thr=10, |ieta|>=17 & |ieta|<=28 thr=9, |ieta|>=29 thr=5, checksums correct.";
    baseConf . iov_begin = "1";
    baseConf . iov_end = "-1";
...
int createRBXLoader(std::string& type_, std::string& tag_,
    ...
    for (std::vector<std::string>::const_iterator _file = brickFileList . begin();
            _file != brickFileList . end(); _file++) {
...
int testocci(void) {
    ...
    const std::string _accessor =
        "occi://CMS_HCL_PRTTYPE_HCAL_READER@anyhost/int2r?PASSWORD=HCAL_Reader_88,LHWM_VERSION=22";
    db -> connect(_accessor);
    std::vector<unsigned int> _lut = db -> getOnlineLUT("gren_P3Thr5", 17, 2, 1, 1,
                                                        0, 1);

    std::cout << "LUT length = " << _lut . size() << std::endl;
    for (std::vector<unsigned int>::const_iterator i = _lut . begin();
            i != _lut . end(); i++) {
...
int test_db_access(void) {
    ...
    try {
        Statement* stmt = _connection -> createStatement();
        std::string query
            = ("SELECT cds.version, CRATE, HTR_SLOT, HTR_FPGA, HTR_FIBER, FIBER_CHANNEL ");
        query += " FROM CMS_HCL_HCAL_CONDITION_OWNER.HCAL_HARDWARE_LOGICAL_MAPS_V3 lmap";
        query += " join cms_hcl_core_condition_owner.cond_data_sets cds ";
        query += " on cds.condition_data_set_id=lmap.condition_data_set_id ";
        query += toolbox::toString(" WHERE SIDE=%d AND ETA=%d AND PHI=%d AND DEPTH=%d AND SUBDETECTOR='%s'",
                                   side, etaAbs, phi, depth, subdetector . c_str());
        ...
        while (rs->next()) {
            ...
            std::cout << _version << "	" << side* etaAbs << "	" << phi << "	" << depth <<
                      "	" << subdetector << "		";
            std::cout << _crate << "	" << _slot << "	" << _fiber << "	" << _channel <<
                      std::endl;
        }
        //Always terminate statement
        _connection -> terminateStatement(stmt);
    ...
    if (infile . is_open()) {
        ...
        while (getline(infile, buf) > 0) {
            ...
            if (_line[0] != "XXXXXX" && _line[1] != "XXXXXX" && _line[2] != "XXXXXX") {
                if (_line[3] != "XXXXXX") {
                    hw_map[_line[0]][_line[1]]["3040000000000" + _line[2]][1] = "3040000000000" +
                                                                                _line[3];
                }
                if (_line[4] != "XXXXXX") {
                    hw_map[_line[0]][_line[1]]["3040000000000" + _line[2]][2] = "3040000000000" +
                                                                                _line[4];
                }
                if (_line[5] != "XXXXXX") {
                    hw_map[_line[0]][_line[1]]["3040000000000" + _line[2]][3] = "3040000000000" +
                                                                                _line[5];
                }
            }
        }
