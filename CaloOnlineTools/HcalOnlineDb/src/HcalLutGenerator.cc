...
void HcalLutGenerator::analyze(const edm::Event& iEvent,
    ...
    std::cout << "EMAP from Event Setup has " << vEmap.size() << " entries" <<
              std::endl;
    ...
    */
    ...
    const int MAXDEPTHHB = 2;
    const int MAXDEPTHHE = 3;
    HcalTopology aHorribleHack(HcalTopologyMode::LHC, MAXDEPTHHB, MAXDEPTHHE);
    HcalLutManager* manager = new HcalLutManager(&aHorribleHack, &(*hEmap), _cq,
    ...
    manager -> createLutXmlFiles_HBEFFromCoder_HOFromAscii_ZDC(_tag, *inputCoder,
                                                               *transcoder, _lin_file, split_by_crate);
    delete manager;
