...
HcalLutSet HcalLutManager::getLutSetFromFile(std::string _filename, int _type) {
    ...
    if (infile . is_open()) {
        ...
        for (std::vector<std::string>::const_iterator iter = buf_vec.begin();
                iter != buf_vec.end(); iter++) {
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getLutXmlFromAsciiMaster(std::string _filename,
                                         std::string _tag, int _crate, bool split_by_crate) {
    std::cout << "Generating linearization (input) LUTs from ascii master file..."
              << std::endl;
    ...
    for (std::map<int, LMapRow>::const_iterator row = _map.begin();
            row != _map.end(); row++) {
        LutXml::Config _cfg;
        ...
        for (int i = 0; i < lut_set_size; i++) {
            if ((row->second.crate == _crate || _crate == -1)
                    &&  // -1 stands for all crates
        ...
        if (lut_index >= 0) {
            if (_xml.count(row->second.crate) == 0 && split_by_crate) {
                _xml.insert(std::pair<int, boost::shared_ptr<LutXml> >(row->second.crate,
                                                                       boost::shared_ptr<LutXml>(new LutXml())));
            }
            else if (_xml.count(0) == 0 && !split_by_crate) {
                _xml.insert(std::pair<int, boost::shared_ptr<LutXml> >(0,
                                                                       boost::shared_ptr<LutXml>(new LutXml())));
            }
            ...
            if (row->second.fpga . find("top") != std::string::npos) {
                ...
                std::cout << "Warning! fpga out of range..." << std::endl;
            }
    ...
    std::cout <<
              "Generating linearization (input) LUTs from ascii master file...DONE" <<
              std::endl;
    return _xml;
}
...
//
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getLinearizationLutXmlFromAsciiMasterEmap(std::string _filename,
                                                          std::string _tag, int _crate, bool split_by_crate) {
    std::cout << "Generating linearization (input) LUTs from ascii master file..."
              << std::endl;
    std::map<int, boost::shared_ptr<LutXml> > _xml; // index - crate number
    ...
    for (std::vector<EMap::EMapRow>::const_iterator row = _map.begin();
            row != _map.end(); row++) {
        if ((row->subdet.find("HB") != string::npos ||
            ...
            if (lut_index >= 0) {
                if (_xml.count(row->crate) == 0 && split_by_crate) {
                    _xml.insert(std::pair<int, boost::shared_ptr<LutXml> >(row->crate,
                                                                           boost::shared_ptr<LutXml>(new LutXml())));
                }
                else if (_xml.count(0) == 0 && !split_by_crate) {
                    _xml.insert(std::pair<int, boost::shared_ptr<LutXml> >(0,
        ...
        if ((_subdet == HcalBarrel ||
            ...
            if (lut_index >= 0) {
                if (_xml.count(aCrate) == 0 && split_by_crate) {
                    _xml.insert(std::pair<int, boost::shared_ptr<LutXml> >(aCrate,
                                                                           boost::shared_ptr<LutXml>(new LutXml())));
                }
                else if (_xml.count(0) == 0 && !split_by_crate) {
                    _xml.insert(std::pair<int, boost::shared_ptr<LutXml> >(0,
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getCompressionLutXmlFromAsciiMaster(std::string _filename,
                                                    std::string _tag, int _crate, bool split_by_crate) {
    std::cout << "Generating compression (output) LUTs from ascii master file..." <<
              std::endl;
    std::map<int, boost::shared_ptr<LutXml> > _xml; // index - crate number
    ...
    std::cout << "LUTs generated: " << _counter.getCount() << std::endl;
    std::cout <<
              "Generating compression (output) LUTs from ascii master file...DONE" <<
              std::endl;
    return _xml;
}
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getLinearizationLutXmlFromCoder(const HcalTPGCoder& _coder,
                                                std::string _tag, bool split_by_crate) {
    std::cout << "Generating linearization (input) LUTs from HcaluLUTTPGCoder..." <<
              std::endl;
    std::map<int, boost::shared_ptr<LutXml> > _xml; // index - crate number
    ...
    for (std::map<int, LMapRow>::const_iterator row = _map.begin();
        ...
        HcalDetId _detid(row->second.det, row->second.side * row->second.eta,
                         row->second.phi, row->second.depth);
        ...
        for (std::vector<unsigned short>::const_iterator _i = coder_lut.begin();
                _i != coder_lut.end(); _i++) {
    ...
    std::cout <<
              "Generating linearization (input) LUTs from HcaluLUTTPGCoder...DONE" <<
              std::endl;
    return _xml;
}
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getLinearizationLutXmlFromCoderEmap(const HcalTPGCoder& _coder,
                                                    std::string _tag, bool split_by_crate) {
    std::cout << "Generating linearization (input) LUTs from HcaluLUTTPGCoder..." <<
              std::endl;
    std::map<int, boost::shared_ptr<LutXml> > _xml; // index - crate number
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getCompressionLutXmlFromCoder(const CaloTPGTranscoderULUT&
                                              _coder, std::string _tag, bool split_by_crate) {
    std::cout << "Generating compression (output) LUTs from CaloTPGTranscoderULUT,"
              << std::endl;
    ...
    for (std::vector<EMap::EMapRow>::const_iterator row = _map.begin();
        ...
        if (row->subdet . find("HT") != std::string::npos && topo_->validHT(detid)) {
            ...
            for (std::vector<unsigned char>::const_iterator _i = coder_lut.begin();
                    _i != coder_lut.end(); _i++) {
                unsigned int _temp = (unsigned int)(*_i);
                //if (_temp!=0) std::cout << "DEBUG non-zero LUT!!!!!!!!!!!!!!!" << (*_i) << "     " << _temp << std::endl;
                //unsigned int _temp = 0;
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::getCompressionLutXmlFromCoder(std::string _tag,
                                              bool split_by_crate) {
    std::cout << "Generating compression (output) LUTs from CaloTPGTranscoderULUT"
              << std::endl;
    std::map<int, boost::shared_ptr<LutXml> > _xml; // index - crate number
    ...
    for (std::vector<EMap::EMapRow>::const_iterator row = _map.begin();
        ...
        if (row->subdet . find("HT") != std::string::npos && topo_->validHT(detid)) {
            ...
            std::vector<unsigned char> coder_lut(1024,
                                                 0); // = _coder.getCompressionLUT(_detid);
            for (std::vector<unsigned char>::const_iterator _i = coder_lut.begin();
                    _i != coder_lut.end(); _i++) {
                unsigned int _temp = (unsigned int)(*_i);
                //if (_temp!=0) std::cout << "DEBUG non-zero LUT!!!!!!!!!!!!!!!" << (*_i) << "     " << _temp << std::endl;
...
int HcalLutManager::writeLutXmlFiles(std::map<int, boost::shared_ptr<LutXml> >&
                                     _xml, std::string _tag, bool split_by_crate) {
    for (std::map<int, boost::shared_ptr<LutXml> >::const_iterator cr =
                _xml.begin(); cr != _xml.end(); cr++) {
...
int HcalLutManager::createLinLutXmlFiles(std::string _tag,
                                         std::string _lin_file, bool split_by_crate) {
    ...
    if (_lin_file.size() != 0) {
        addLutMap(xml, getLinearizationLutXmlFromAsciiMasterEmap(_lin_file, _tag, -1,
                                                                 split_by_crate));
    }
...
int HcalLutManager::createAllLutXmlFiles(std::string _tag,
                                         std::string _lin_file, std::string _comp_file, bool split_by_crate) {
    //std::cout << "DEBUG1: split_by_crate = " << split_by_crate << std::endl;
    std::map<int, boost::shared_ptr<LutXml> > xml;
    if (!lut_checksums_xml) {
        lut_checksums_xml = new XMLDOMBlock("CFGBrick", 1);
    ...
    if (_comp_file.size() != 0) {
        ...
        addLutMap(xml, getCompressionLutXmlFromAsciiMaster(_comp_file, _tag, -1,
                                                           split_by_crate));
...
int HcalLutManager::createAllLutXmlFilesFromCoder(const HcalTPGCoder& _coder,
    ...
    addLutMap(xml, getLinearizationLutXmlFromCoderEmap(_coder, _tag,
                                                       split_by_crate));
    addLutMap(xml, getCompressionLutXmlFromCoder(_tag, split_by_crate));
...
int HcalLutManager::createLutXmlFiles_HBEFFromCoder_HOFromAscii(
    std::string _tag, const HcalTPGCoder& _coder,
    const CaloTPGTranscoderULUT& _transcoder, std::string _lin_file,
    bool split_by_crate) {
    std::map<int, boost::shared_ptr<LutXml> > xml;
    if (!lut_checksums_xml) {
        lut_checksums_xml = new XMLDOMBlock("CFGBrick", 1);
...
int HcalLutManager::createLutXmlFiles_HBEFFromCoder_HOFromAscii(
    std::string _tag, const HcalTPGCoder& _coder, std::string _lin_file,
    bool split_by_crate) {
    std::map<int, boost::shared_ptr<LutXml> > xml;
    if (!lut_checksums_xml) {
        lut_checksums_xml = new XMLDOMBlock("CFGBrick", 1);
    ...
    //
    const std::map<int, boost::shared_ptr<LutXml> > _comp_lut_xml =
        getCompressionLutXmlFromCoder(_tag, split_by_crate);
    addLutMap(xml, _comp_lut_xml);
...
void HcalLutManager::addLutMap(std::map<int, boost::shared_ptr<LutXml> >&
                               result, const std::map<int, boost::shared_ptr<LutXml> >& other) {
    for (std::map<int, boost::shared_ptr<LutXml> >::const_iterator lut =
                other.begin(); lut != other.end(); lut++) {
...
int HcalLutManager::test_xml_access(std::string _tag, std::string _filename) {
    ...
    _lut = getLutFromXml(_tag, 1107313727,
    ...
    gettimeofday(&_t, NULL);
    std::cout << "parsing took that much time: " << (double)(_t . tv_sec) +
              (double)(_t . tv_usec) / 1000000.0 - _time << std::endl;
    ...
    for (std::vector<unsigned int>::const_iterator i = _lut . end() - 1;
            i != _lut . begin() - 1; i--) {
        std::cout << (i - _lut.begin()) << "     " << _lut[(i - _lut.begin())] <<
                  std::endl;
        break;
    }
...
int HcalLutManager::read_lmap(std::string lmap_hbef_file,
    ...
    std::cout << "LMap contains " << lmap -> get_map() . size() <<
...
int HcalLutManager::local_connect(std::string lut_xml_file,
                                  std::string lmap_hbef_file, std::string lmap_ho_file) {
...
std::vector<unsigned int> HcalLutManager::getLutFromXml(std::string tag,
                                                        uint32_t _rawid, hcal::ConfigurationDatabase::LUTType _lt) {
    std::cout <<
...
std::vector<unsigned int> HcalLutManager::getLutFromXml_old(std::string tag,
                                                            uint32_t _rawid, hcal::ConfigurationDatabase::LUTType _lt) {
    if (!lmap) {
        std::cout << "HcalLutManager: cannot find LUT without LMAP, exiting..." <<
                  std::endl;
        exit(-1);
    }
    if (!db) {
        std::cout <<
                  "HcalLutManager: cannot find LUT, no source (local XML file), exiting..." <<
                  std::endl;
        exit(-1);
    }
    ...
    if (_map . find(_rawid) != _map.end()) {
        ...
        if (_fpga . find("top") != std::string::npos) {
            ...
            std::cout <<
                      "HcalLutManager: irregular LMAP fpga value... do not know what to do - exiting"
                      << std::endl;
            exit(-1);
        }
        ...
        result = db -> getOnlineLUT(tag, _crate, _slot, topbottom, _fiber, _channel,
                                    luttype);
    }
...
int HcalLutManager::get_xml_files_from_db(std::string tag,
                                          const std::string db_accessor, bool split_by_crate) {
    std::map<int, boost::shared_ptr<LutXml> > lut_map = get_brickSet_from_oracle(
                                                            tag, db_accessor);
    if (split_by_crate) {
        ...
        LutXml result;
        for (std::map<int, boost::shared_ptr<LutXml> >::const_iterator xml =
                    lut_map.begin(); xml != lut_map.end(); xml++) {
            result += *(xml->second);
        }
...
std::map<int, boost::shared_ptr<LutXml> >
HcalLutManager::get_brickSet_from_oracle(std::string tag,
    ...
    std::cout << "Preparing to request the LUT CLOBs from the database..." <<
              std::endl;
        ...
        while (rs->next()) {
            ...
            if (crate != -1) {  // not a brick with checksums
                ...
                */
                ...
                MemBufInputSource* lut_clob = new MemBufInputSource((const XMLByte*)bs,
                                                                    strlen(bs), "lut_clob", false);
                boost::shared_ptr<LutXml> lut_xml = boost::shared_ptr<LutXml>(new LutXml(
                                                                                  *lut_clob));
...
int HcalLutManager::create_lut_loader(std::string file_list,
                                      std::string _prefix, std::string tag_name, std::string comment,
                                      std::string version, int subversion) {
    ...
    for (std::vector<std::string>::const_iterator _f = file_name.begin();
            _f != file_name.end(); _f++) {
        int crate_begin = _f->rfind("_");
        int crate_end = _f->rfind(".xml.dat");
        crate_number . push_back(getInt(_f->substr(crate_begin + 1,
                                                   crate_end - crate_begin - 1)));
    }
    //
    ...
    CSconf.version = conf.version;
    //
    for (std::vector<std::string>::const_iterator _file = file_name . begin();
            _file != file_name . end(); _file++) {
...
int HcalLutManager::createLutXmlFiles_HBEFFromCoder_HOFromAscii_ZDC(
    ...
    addLutMap(xml, _comp_lut_xml);
    //
    const std::map<int, boost::shared_ptr<LutXml> > _zdc_lut_xml = getZdcLutXml(
                                                                       _tag, split_by_crate);
...
std::map<int, boost::shared_ptr<LutXml> > HcalLutManager::getZdcLutXml(
    std::string _tag,
    bool split_by_crate) {
    ...
    for (std::vector<EMap::EMapRow>::const_iterator row = _map.begin();
        ...
        if (row->zdc_section . find("ZDC") != std::string::npos) {
            ...
            if (coder_lut.size() != 0) {
                for (std::vector<int>::const_iterator _i = coder_lut.begin();
                        _i != coder_lut.end(); _i++) {
                    unsigned int _temp = (unsigned int)(*_i);
                    //if (_temp!=0) std::cout << "DEBUG non-zero LUT!!!!!!!!!!!!!!!" << (*_i) << "     " << _temp << std::endl;
                    //unsigned int _temp = 0;
